    ...
    return (
        <div style={{display: 'flex', alignItems: 'center', flexDirection: 'column', margin: '6rem 2rem'}}>
            <div className={cx('mainRight')}>
                <button style={{padding: '1rem 2rem', backgroundColor: '#d8355a', width: '135px', color: '#fff', cursor: 'pointer'}} onClick={() => {setIsModalVoucher(true);setOpenModal(true)}}>
                    Thêm mới
                </button>
            ...
            {openModal&&<Modal onCloseModal={onCloseModal} title="Thông tin Voucher" onSubmit={handleAddNewVoucher}  isModalConfirm={openModalConfirm} isModalVoucher={isModalVoucher}/>}
        </div>
    )
}
